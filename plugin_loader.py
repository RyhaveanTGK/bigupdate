...
def load_plugin_file(client, path, name=None):
    """Import a single ``.py`` plugin file and register its handlers on
    ``client`` immediately (no restart needed). Raises on failure so the
    caller (``.pinstall``) can report the exact error."""
    path = Path(path)
